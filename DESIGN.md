...
Below the field the ground picks the page back up: the one-sentence claim, then the three-metric ruled row, then a thin marquee band. The two field regions are separated by ground rather than touching, which is the same fix the Stack/footer boundary needed. That metric row is a three-column grid whose dividers are 1px left borders with 2rem of inset; under 640px it collapses to one column and the dividers rotate to top borders with 1.5rem of inset. Projects sit two-up from 768px.
